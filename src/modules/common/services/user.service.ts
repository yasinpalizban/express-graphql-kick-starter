...
export default class UserService implements ServiceInterface {
  ...
  public async index(paginateSearch: PaginateSearch): Promise<IUserPagination> {
    ...
    const pipeLine: AggregatePipeLine[] = paginateSearch.decodeQueryParam().getPipeLine(defaultPipeLine);

    // @ts-ignore
    let pagination: IPagination = await this.userModel.aggregatePaginate(this.userModel.aggregate(pipeLine), paginateSearch.OptionPagination);
